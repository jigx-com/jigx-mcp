...
export async function handleGetOrganizationMember(args: unknown, deps: HandlerDeps): Promise<CallToolResult> {
  const log = deps.logger
  const start = Date.now()
  log.info({ args }, '[MCP] handleGetOrganizationMember: start')
  try {
