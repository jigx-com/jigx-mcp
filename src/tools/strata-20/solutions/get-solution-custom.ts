...
const title = 'Get Solution Custom Settings'

// Tool definition
export const getSolutionCustomTool: Tool = {
  name: 'get_solution_custom',
  title,
  description: 'Get solution custom settings',
...
// Handler function
export async function handleGetSolutionCustom(args: unknown, deps: HandlerDeps): Promise<CallToolResult> {
  const log = deps.logger
  const start = Date.now()
  log.info({ args }, '[MCP] handleGetSolutionCustom: start')
  try {
    ...
    const response: CallToolResult = await withRetry(async () => {
      ...
      if (res.ok) {
        ...
        log.info('[MCP] handleGetSolutionCustom: success', { duration: Date.now() - start })
        return {
          content: [{
            type: 'text',
    ...
    log.error({ error }, '[MCP] handleGetSolutionCustom: error')
